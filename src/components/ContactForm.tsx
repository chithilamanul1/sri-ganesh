  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white/60 border border-glass-border p-8 shadow-xl sm:p-10 lg:p-12 rounded-3xl backdrop-blur-md relative overflow-hidden group">
      ...
      <div className="mt-8 grid gap-x-6 gap-y-8 sm:grid-cols-2">
        <Field label="Your name *"><input required name="name" autoComplete="name" className="form-control bg-white/50 text-brown-dark placeholder:text-slate-400 focus:bg-white" placeholder="Full name" /></Field>
        <Field label="Company *"><input required name="company" autoComplete="organization" className="form-control bg-white/50 text-brown-dark placeholder:text-slate-400 focus:bg-white" placeholder="Company name" /></Field>
        <Field label="Work email *"><input required type="email" name="email" autoComplete="email" className="form-control bg-white/50 text-brown-dark placeholder:text-slate-400 focus:bg-white" placeholder="you@company.com" /></Field>
        
        <label className="block text-[10px] font-bold uppercase tracking-[0.15em] text-green-dark">
          Product interest *
          <span className="relative mt-3 block">
            <select required name="product" defaultValue="" className="form-control bg-white/50 text-brown-dark appearance-none cursor-pointer focus:bg-white">
              <option value="" disabled className="text-slate-500">Select a product</option>
              {products.map((product) => (<option key={product.name} className="bg-white text-brown-dark">{product.name}</option>))}
              <option className="bg-white text-brown-dark">Multiple products</option>
            </select>
      ...
      <label className="mt-8 block text-[10px] font-bold uppercase leading-none tracking-[0.15em] text-green-dark">
        Your requirements
        <textarea name="message" rows={3} className="form-control bg-white/50 text-brown-dark placeholder:text-slate-400 focus:bg-white mt-3 resize-none" placeholder="Quantity, preferred grade, destination, or other details" />
      </label>
