...
export function ContactForm() {
  ...
  if (submitted) {
    return (
      <div className="flex min-h-[420px] flex-col items-center justify-center bg-glass border border-glass-border p-8 text-center rounded-3xl backdrop-blur-md">
        <span className="flex h-16 w-16 items-center justify-center rounded-full bg-teal/10 text-teal border border-teal/20 shadow-[0_0_20px_rgba(8,247,190,0.2)]">
        ...
        <button type="button" onClick={() => setSubmitted(false)} className="mt-8 border-b border-teal pb-1 text-xs font-bold uppercase tracking-[0.15em] text-teal hover:text-white transition-colors">
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-glass border border-glass-border p-8 shadow-2xl sm:p-10 lg:p-12 rounded-3xl backdrop-blur-md relative overflow-hidden group">
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-teal/20 via-teal to-teal/20 transform -translate-x-full group-hover:translate-x-0 transition-transform duration-700" />
      ...
      <div className="mt-8 grid gap-x-6 gap-y-8 sm:grid-cols-2">
        ...
        <label className="block text-[10px] font-bold uppercase tracking-[0.15em] text-teal">
          Product interest *
          <span className="relative mt-3 block">
            <select required name="product" defaultValue="" className="form-control appearance-none cursor-pointer">
              <option value="" disabled>Select a product</option>
              {products.map((product) => (<option key={product.name} className="bg-ocean text-white">{product.name}</option>))}
              <option className="bg-ocean text-white">Multiple products</option>
            </select>
            <ChevronDownIcon className="pointer-events-none absolute right-0 top-3 text-teal" size={16} aria-hidden="true" />
          </span>
        </label>
      </div>
      
      <label className="mt-8 block text-[10px] font-bold uppercase leading-none tracking-[0.15em] text-teal">
        Your requirements
      ...
      <button type="submit" className="mt-10 inline-flex items-center gap-3 bg-teal px-8 py-4 text-xs font-bold uppercase tracking-[0.15em] text-ocean-dark transition hover:bg-teal-light rounded-full hover:scale-105 hover:shadow-[0_0_20px_rgba(8,247,190,0.3)] shadow-lg w-full sm:w-auto justify-center">
