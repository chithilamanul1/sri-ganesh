      ...
      <main>
              ...
              {products.map((product, index) => (
                <motion.article key={product.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className={`flex h-full flex-col overflow-hidden bg-peach-light rounded-md shadow-sm ${index === 0 ? 'lg:col-span-2' : ''}`}>
                  ...
                  <div className="flex flex-1 flex-col p-6 sm:p-7">
                    ...
                    <Link href="/contact" className="mt-6 inline-flex items-center gap-2 self-start text-xs font-extrabold uppercase leading-none tracking-[0.11em] text-brown hover:text-green">
        ...
        <section className="bg-brown py-16 text-center text-white">
          <div className="mx-auto max-w-2xl px-5">
            ...
            <Link href="/contact" className="mt-7 inline-flex bg-green px-6 py-4 text-xs font-extrabold uppercase tracking-[0.13em] text-white transition hover:bg-green-dark">
              Send an enquiry
            </Link>
          </div>
        </section>
