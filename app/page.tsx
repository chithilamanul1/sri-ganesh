...
export default function HomePage() {
  return (
    <div className="min-h-screen bg-ocean text-slate-200 font-sans selection:bg-teal selection:text-ocean-dark">
      <main>
        {/* Hero Section */}
        <section className="relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden pt-24 pb-32 md:pb-40">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-ocean-dark/90 via-ocean/60 to-ocean" aria-hidden="true" />
          ...
          <div className="relative z-10 mx-auto max-w-5xl px-5 sm:px-8 flex flex-col items-center w-full my-auto text-center">
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}>
              <div className="flex items-center justify-center gap-4 mb-8">
                <span className="h-[1px] w-12 sm:w-20 bg-gradient-to-r from-transparent to-teal" /> 
                <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.4em] text-teal">
                  Premium Marine Products
                </p>
                <span className="h-[1px] w-12 sm:w-20 bg-gradient-to-l from-transparent to-teal" />
              </div>
              <h1 className="font-serif text-5xl leading-[1.1] text-white mb-6 sm:text-7xl lg:text-8xl drop-shadow-2xl">
                ...
                <span className="italic bg-clip-text text-transparent bg-gradient-to-r from-teal-light to-teal">Ocean Harvest</span>
              </h1>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                <Link href="/products" className="group relative overflow-hidden rounded-full bg-teal px-8 py-4 text-xs font-bold uppercase tracking-[0.15em] text-ocean-dark transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(8,247,190,0.4)] w-full sm:w-auto">
            ...
            <div className="grid grid-cols-4 bg-glass backdrop-blur-xl border-t border-x border-glass-border rounded-t-2xl divide-x divide-glass-border shadow-2xl">
              {[['05', 'Specialist Products'], ['01', 'Island Source'], ['B2B', 'Export Focused'], ['Global', 'Market Ready']].map(([value, label]) => (
                <div key={label} className="px-6 py-8 text-center group cursor-default transition-colors hover:bg-white/5">
                  <p className="font-serif text-4xl text-teal-light transition-transform duration-500 group-hover:scale-110">{value}</p>
        ...
        <section className="py-24 sm:py-32 bg-ocean relative overflow-hidden">
          <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-teal/5 to-transparent blur-3xl opacity-50 pointer-events-none" />
          ...
          <div className="mx-auto grid max-w-7xl items-center gap-16 px-5 sm:px-8 lg:grid-cols-2 lg:px-10 relative z-10">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: "easeOut" }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-br from-teal to-coral/30 rounded-2xl opacity-20 blur-xl transition-all duration-700 group-hover:opacity-40" />
              <div className="relative overflow-hidden rounded-2xl shadow-2xl aspect-[4/5] sm:aspect-square md:aspect-[4/5]">
                <img src="/image3.JPG" alt="Boat travelling across Sri Lankan ocean waters" className="absolute inset-0 h-full w-full object-cover transition-transform duration-1000 group-hover:scale-110 grayscale hover:grayscale-0" />
                <div className="absolute inset-0 bg-ocean-dark/20 mix-blend-multiply transition-opacity duration-700 group-hover:opacity-0" />
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: "easeOut" }}>
              <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full border border-teal/30 bg-teal/5 mb-8">
                ...
                <p className="text-[10px] font-extrabold uppercase leading-none tracking-[0.25em] text-teal">The Sri Ganesh Standard</p>
              </div>
              ...
              <div className="mt-12 grid gap-6 sm:grid-cols-2">
                {['Reliable supplier relationships', 'Specification-led grading', 'Export-ready documentation', 'Flexible order discussions'].map((item, i) => (
                  ...
                  >
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-teal/10 text-teal mt-0.5 border border-teal/20 transition-transform group-hover:scale-110 group-hover:bg-teal group-hover:text-ocean-dark">
                      <CheckIcon size={14} />
                    </span> 
              ...
              <Link href="/about" className="group mt-14 inline-flex items-center gap-3 text-xs font-bold uppercase tracking-[0.15em] text-teal hover:text-teal-light transition-colors relative">
                Discover our story 
                <span className="relative flex h-8 w-8 items-center justify-center rounded-full bg-teal/10 transition-transform group-hover:translate-x-2">
        ...
        <section className="bg-ocean-dark py-24 sm:py-32 relative">
          ...
          <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
            <div className="flex flex-col text-center lg:text-left lg:flex-row justify-between gap-6 pb-12 items-end">
              <div>
                <p className="text-xs font-extrabold uppercase tracking-[0.25em] text-teal">Our Collection</p>
              ...
              <Link href="/products" className="group inline-flex items-center justify-center gap-3 rounded-full border border-glass-border px-6 py-3 text-xs font-bold uppercase tracking-[0.1em] text-slate-300 hover:text-white hover:bg-glass transition-all">
                View full catalogue <ArrowRightIcon size={14} className="transition-transform group-hover:translate-x-1 text-teal" />
              </Link>
            </div>
            ...
            <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {products.slice(0, 3).map((product, i) => (
                <motion.article 
                  ...
                  className="group flex flex-col overflow-hidden rounded-2xl bg-ocean border border-glass-border relative transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(8,247,190,0.2)] hover:border-teal/30"
                >
                  <div className="overflow-hidden relative h-72">
                    <div className="absolute inset-0 bg-ocean-dark/40 group-hover:bg-transparent transition-colors z-10 duration-500" />
                    ...
                    <div className="absolute top-4 right-4 z-20 rounded-full border border-glass-border bg-glass backdrop-blur-md px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-teal shadow-2xl">
                      {product.tag}
                    </div>
                  </div>
                  <div className="p-8 flex flex-col flex-grow relative z-20 bg-gradient-to-t from-ocean via-ocean to-ocean/95">
                    <h3 className="font-serif text-2xl text-white group-hover:text-teal-light transition-colors">{product.name}</h3>
                    ...
                    <Link href="/contact" className="mt-auto inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.1em] text-teal hover:text-white transition-colors w-fit group/btn">
                      Request details 
                      <span className="block h-[1px] w-4 bg-teal transition-all group-hover/btn:w-8" />
        ...
        <section className="bg-ocean py-24 sm:py-32 relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-teal/5 via-ocean to-ocean blur-2xl" />
          ...
          <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10 relative z-10">
            <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
              ...
              >
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-teal/10 border border-teal/20 text-teal shadow-[0_0_20px_rgba(8,247,190,0.15)]">
              ...
              <motion.div 
                ...
                className="flex flex-col gap-6 items-start rounded-3xl border border-glass-border bg-glass backdrop-blur-xl p-10 hover:bg-white/5 transition-colors"
              >
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-coral/10 border border-coral/20 text-coral shadow-[0_0_20px_rgba(255,111,97,0.15)]">
