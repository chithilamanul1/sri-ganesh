  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-700">
      ...
      <main>
        ...
        <section className="bg-transparent relative overflow-hidden py-20 sm:py-32">
          ...
          <div className="mx-auto grid max-w-7xl gap-16 px-5 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20 lg:px-10 relative z-10">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
              <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-green-dark">Built for confident trade</p>
              <h2 className="mt-4 font-serif text-4xl leading-tight text-brown-dark sm:text-5xl">Direct, considered, and responsive.</h2>
              <div className="mt-10 flex gap-5 rounded-2xl border border-glass-border bg-white/60 p-6 backdrop-blur-md shadow-sm transition-colors">
                <ShieldCheckIcon className="shrink-0 text-green-dark w-8 h-8" />
                <p className="text-[15px] leading-relaxed text-slate-600 font-light pt-1">
            ...
            <ol className="border-t border-glass-border">
              {stages.map(([number, title, text], i) => (
                <motion.li 
                  ...
                  key={number} className="group grid grid-cols-[80px_1fr] gap-6 border-b border-glass-border py-10 transition-colors hover:bg-white/30 px-4 -mx-4 rounded-xl"
                >
                  <span className="font-serif text-4xl text-green/60 transition-colors group-hover:text-green-dark pt-1">{number}</span>
                  <div>
                    <h3 className="text-xl font-serif text-brown-dark group-hover:text-green-dark transition-colors">{title}</h3>
                    <p className="mt-3 max-w-lg text-[15px] leading-relaxed text-slate-600 font-light">{text}</p>
                  </div>
                </motion.li>
              ))}
            </ol>
        ...
        <section className="bg-transparent border-t border-glass-border py-24 sm:py-32 relative">
          ...
          <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10 relative z-10">
            <div className="grid gap-12 lg:gap-8 md:grid-cols-3">
              {[
                ...
                <motion.div 
                  ...
                  key={title} className="rounded-3xl border border-glass-border bg-white/60 p-8 backdrop-blur-md relative overflow-hidden group shadow-md"
                >
                  ...
                  <h2 className="font-serif text-2xl text-brown-dark mb-4 group-hover:text-green-dark transition-colors">{title}</h2>
                  <p className="text-[15px] leading-relaxed text-slate-600 font-light">{text}</p>
                </motion.div>
              ))}
            </div>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.6 }} className="mt-16 text-center">
              <Link href="/contact" className="group inline-flex items-center justify-center gap-3 rounded-full bg-green-dark px-8 py-4 text-xs font-bold uppercase tracking-[0.15em] text-white shadow-md transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(44,110,67,0.3)]">
